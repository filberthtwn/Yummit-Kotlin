...
class DeliveryVoucherViewModel(application: Application): BaseViewModel(application){
    val loading = MutableLiveData<Boolean>()
    val data = MutableLiveData<List<Voucher>>()

    fun getDeliveryVoucher(){
        loading.value = true


//        disposable.add(
//            apiService.getDeliveryVoucher()
...
class ClosingHoursFoodViewModel(application: Application): BaseViewModel(application){
    val loading = MutableLiveData<Boolean>()
    val data = MutableLiveData<List<Food>>()

    fun getClosingHoursFood(){
        loading.value = true
//        disposable.add(
//            apiService.getClosingHoursFood()
//                .subscribeOn(Schedulers.newThread())
//                .observeOn(AndroidSchedulers.mainThread())
...
class FastAndHungriousViewModel(application: Application): BaseViewModel(application){
    val loading = MutableLiveData<Boolean>()
    val data = MutableLiveData<List<Restaurant>>()
    val gson = Gson()

    fun getFastAndHungrious(){
        loading.value = true
